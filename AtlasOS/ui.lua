...
local AtlasOS_HOME = "/home/AtlasOS"
...
--- True if path is /home/AtlasOS or inside it (system AtlasOS tree).
local function path_is_AtlasOS_tree(p)
  p = path_normalize(p)
  return p == AtlasOS_HOME or p:sub(1, #AtlasOS_HOME + 1) == AtlasOS_HOME .. "/"
end
...
function UI.files_show_list_entry(parent_dir, entry_name, is_dir)
  ...
  if path_normalize(parent_dir) == "/home" and entry_name == "AtlasOS" then
    return false
  end
  return true
end
...
function UI.files_effective_dir()
  ...
  if path_is_AtlasOS_tree(d) and not UI.developer_mode_enabled() then
    return "/home"
  end
  return d
end
...
function UI.settings_dispatch(tag)
  ...
  elseif tag == "dev:toggle" then
    ...
    if path_is_AtlasOS_tree(path_normalize(UI.files_dir or "")) then
      UI.files_dir = "/home"
    end
...
function UI.files_set_dir(path)
  ...
  if path_is_AtlasOS_tree(path) and not UI.developer_mode_enabled() then
    _G.AtlasOS_log = _G.AtlasOS_log or {}
...
function UI.boot()
  ...
  if path_is_AtlasOS_tree(path_normalize(UI.files_dir or "")) and not UI.developer_mode_enabled() then
    UI.files_dir = "/home"
  end
  UI.build_desktop()
  UI.redraw()
