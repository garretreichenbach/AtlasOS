# DovOS app packages (`appinfo.json`)
...
| `args` | array | Strings/numbers passed to the app as **`_G.DovOS_APP.args`** (table of strings). |
| `window` | string | Reserved for future DovOS window integration. |
| `dovos` | object | Reserved for future flags (e.g. sandbox, permissions). |

## DovOS system packages (`/home/dovos/apps/`)
...
Then any **other** subfolder of `/home/dovos/apps/` is loaded (by id). User packages under `/home/apps/` **cannot** replace ids registered from `/home/dovos/apps/`.
...
- `/home/dovos/apps/files/appinfo.json` + `explorer.lua` + `main.lua`
- `/home/dovos/apps/settings/appinfo.json` + `settings_ui.lua` (categories + controls)
- `/home/dovos/apps/search/appinfo.json` + `search_engine.lua` + `main.lua`
...
- **`_G.DovOS_APP`** is set to `{ id, package_dir, args }` where `args` is an array of strings from `appinfo.args`.
- After the script returns, `DovOS_APP` is cleared.
